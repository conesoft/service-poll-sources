...
namespace Conesoft.Services.PollSources.Features.Weather.Services;

class PeriodicPoll(OpenMeteoClient client, HostEnvironment environment) : PeriodicStorage<CachedWeatherData>(environment, "data/weather/forecast.json", TimeSpan.FromMinutes(5))
{
...
abstract class PeriodicStorage<T>(HostEnvironment environment, string path, TimeSpan period) : PeriodicCache<T>(period) where T : class
{
    protected override async Task OnContentChanged(T content)
    {
        var storage = environment.Global.Storage / Filename.FromExtended(path);
        await storage!.WriteAsJson(content);
    }
}
